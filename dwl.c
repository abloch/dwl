...
dwl_ipc_output_printstatus_to(DwlIpcOutput *ipc_output)
{
	...
	if (wl_resource_get_version(ipc_output->resource) >= ZDWL_IPC_OUTPUT_V2_CLIENT_SINCE_VERSION) {
		wl_list_for_each(c, &clients, link) {
			if (c->mon != monitor)
				continue;
			if (!c->foreign_toplevel || !c->foreign_toplevel->identifier)
				continue;
			zdwl_ipc_output_v2_send_client(ipc_output->resource,
				c->foreign_toplevel->identifier,
				c->tags,
				c == focused ? 1 : 0,
				c->isurgent,
				c->isfloating,
...
mapnotify(struct wl_listener *listener, void *data)
{
	...
	/* Publish this toplevel via ext_foreign_toplevel_list_v1. */
	c->foreign_toplevel = wlr_ext_foreign_toplevel_handle_v1_create(foreign_toplevel_list,
...
updatetitle(struct wl_listener *listener, void *data)
{
	...
	if (c->foreign_toplevel)
		wlr_ext_foreign_toplevel_handle_v1_update_state(c->foreign_toplevel,
				&(struct wlr_ext_foreign_toplevel_handle_v1_state){
					.title = client_get_title(c),
					.app_id = client_get_appid(c),
				});
